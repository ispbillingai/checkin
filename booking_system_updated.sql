...
-- Create entry points table if not exists
...
-- Create room_entry_points table if not exists (for mapping rooms to entry points)
...
-- Create entry_point_pins table if not exists (for storing multiple entry points per booking with positions)
...
-- Insert default entry points if not exists
...
-- Map rooms to entry points if not exists
...
VALUES (
    1, 
    0, 
    '<html><head><title>Your Booking Confirmation</title></head><body><h2>Booking Confirmation</h2><p>Dear {GUEST_NAME},</p><p>Your booking has been confirmed with the following details:</p><table style="border-collapse: collapse; width: 100%;"><tr><td style="border: 1px solid #ddd; padding: 8px;"><strong>Room:</strong></td><td style="border: 1px solid #ddd; padding: 8px;">{ROOM_NAME}</td></tr><tr><td style="border: 1px solid #ddd; padding: 8px;"><strong>Check-in:</strong></td><td style="border: 1px solid #ddd; padding: 8px;">{ARRIVAL_DATETIME}</td></tr><tr><td style="border: 1px solid #ddd; padding: 8px;"><strong>Check-out:</strong></td><td style="border: 1px solid #ddd; padding: 8px;">{DEPARTURE_DATETIME}</td></tr><tr><td style="border: 1px solid #ddd; padding: 8px;"><strong>Access Code:</strong></td><td style="border: 1px solid #ddd; padding: 8px;"><h3>{ACCESS_CODE}</h3></td></tr></table><p>Please use this access code at the specified entry points during your stay.</p><p>Thank you for choosing our service!</p></body></html>', 
    'Your booking at {ROOM_NAME} is confirmed. Access code: {ACCESS_CODE}. Check-in: {ARRIVAL_DATETIME}.', 
    'smtp.example.com', 
    587
) ON DUPLICATE KEY UPDATE id = id;
